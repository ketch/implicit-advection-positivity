...
Hence, in what follows, we can assume $\theta \in (0,1]$.

As we will see, similar results described in Section~\ref{sectioncentered} hold true for higher-order centered
...
As seen previously, matrix $M$ is a real, circulant matrix so it can be characterized by the entries of its first row,
which take the form
...
The above observation can be extended to any arbitrary even number $m$.
The following proposition proves that for the $4^\text{th}$-order scheme, if the CFL number $\nu$ is
sufficiently large then $M(m,\theta,\nu) \ge 0$ cannot not hold for any $\theta \in (0,1]$ and any arbitrary even
number $m \ge 6$,
A similar result can be shown for the $6^\text{th}$-order scheme as well.
...
\begin{proof}
	...
	$\sin\left(k\frac{2\pi(\ell-1)}{m}\right)=-\sin\left(k\frac{2\pi(m-\ell+2-1)}{m}\right)$ and
	$\cos\left(\frac{2\pi(\ell-1)}{m}\right)=\cos\left(k\frac{2\pi(m-\ell+2-1)}{m}\right)$ for positive integers
	$k$, $\ell$ and $m$).
	Define the function
	\begin{align*}
		f(x; \theta, \nu) \coloneqq \frac{\psi(x)(\sin(x)+\theta\nu\psi(x)\cos(x))}{1+\theta^2\nu^2\psi^2(x)}.
	\end{align*}
	Then, we can express \eqref{M1m} by summing only over indices $\ell$ for which $0 < \xi_\ell  \le \pi/2$,
	yielding
	\begin{align}\label{M1mnew}
		M_{1,m} = \begin{cases}
							-\dfrac{2\nu}{m} \left(\displaystyle{\sum\limits_{\ell=2}^{m/4}} f(\xi_\ell; \theta, \nu) +
								f(\pi-\xi_\ell; \theta, \nu) + \dfrac{\psi(\pi/2)}{1+\theta^2\nu^2\psi^2(\pi/2)}\right),
								&\; \text{if } m \equiv 0 \Mod{4}, \\[20pt]
							-\dfrac{2\nu}{m} \displaystyle{\sum\limits_{\ell=2}^{(m+2)/4}}
								f(\xi_\ell; \theta, \nu) + f(\pi-\xi_\ell; \theta, \nu), &\; \text{if } m \equiv 2 \Mod{4}.
						\end{cases}
	\end{align}
...
%	index $\ell$ in \eqref{M1mnew}.
	For the $4^\text{th}$-order centered spatial discretization easy calculations show that
	\begin{align*}
		\psi(\xi_\ell) = \frac{1}{3}\sin(\xi_\ell)(4 - \cos(\xi_\ell)), \qquad
		\psi(\pi - \xi_\ell) = \frac{1}{3}\sin(\xi_\ell)(4 + \cos(\xi_\ell)),
	\end{align*}
	and hence $\psi(\xi_\ell)$, $\psi(\pi-\xi_\ell)$, as well as $\sin(\xi_\ell)$, and $\cos(\xi_\ell)$ are positive for
	each index $\ell$ in \eqref{M1mnew}.
	...
	This means that if $\nu \ge \nu_0$, then $M_{1,m} < 0$ for any $\theta \in (0,1]$.
\end{proof}
\begin{remark}
	We conjecture that a stronger statement than Proposition~\ref{m_even_nonpositivityFE} holds; given an
	arbitrary finite difference centered discretization in space, coupled with the $\theta$-method then
