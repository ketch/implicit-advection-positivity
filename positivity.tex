...
\begin{corollary} (i) For any $k\in\nplus$ and $\te\in(0,1]$, we have $\nur<\nu_R(k+1,\theta)$, and
\[
...
\]
\end{corollary}
\begin{proof} All these results 
are, again, simple consequences of Lemmas  \ref{lem3} and \ref{lem4}, and \eqref{cor4bijection}. 
For fixed $k\in\nplus$, evaluating the one-sided limit $\lim_{\te\to 0+0} \nul$ would be non-trivial, since in \eqref{nuldef} we have $\frac{2\yl}{1-\yl^2}\to 0$ (due to Lemma \ref{lem4}), while $\frac{1}{\te}\to+\infty$ ($\te\to0+0$).
\end{proof}
...
\caption{A typical shaded region from Figure \ref{fig_variousk} for which $M(2k+1,\te,\nu)\ge 0$ (in this particular case, for $k=1$). The region is described by the inequalities $\nur\le\nu\le\nul$. The upper curve has a vertical asymptote at $\te=2k/(2k+1)$.}\label{fig_boundary}
\end{center}
\end{figure}
...
\[
 \nur>\nul.
\] 
Moreover, for any fixed $k\in\nplus$, there is a unique $\te_k\in[\frac{1}{2},1)$  such that 
\[
\nur \le \nul\quad \Longleftrightarrow \quad \te\ge \te_k.
\]
\end{lemma}
\begin{proof}
\end{proof}
...
\begin{remark}
The ``lower left corner point'' of each shaded region in Figure \ref{fig_variousk} corresponds to a pair $(\te,\nu)$  for which $\nur=\nul$. This means that the leftmost and the rightmost entries of the first row of $M(2k+1,\te,\nu)$ vanish simultaneously. For $k=1$, this happens for $\te=1/2$ and $\nu=4$, when
\[
...
\begin{theorem}
hence $M(2k+1,\te,\nu)\ge 0$ cannot hold for any $k\in\nplus$, $0<\te<1/2$ and $\nu>0$.\\ 

DESCRIBING $M(2k+1,\te,\nu)\ge 0 $ FOR FIXED VALUES OF $\te$.
DIFFERENT CASES TO MENTION:
$\te=0$ (the explicit Euler method), $0<\te<1/2$, $\te=1/2$ (second order method), $1/2<\te<1$, $\te=1$ (the implicit Euler method). FINITELY MANY $k$ VALUES. FOR FIXED K: THETA SMALL then bounded in nu, theta larger, lower but not upper bounded in nu.
\end{theorem}
