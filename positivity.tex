...
\boxed{\text{LL: The intro should be rewritten to take into account the results of the last sections as well.}}\\
\boxed{\text{LL: Mention somewhere: the matrices $L$ stand for spatial discretization, $M$ for the full discretization matrices (terminology???)}}\\
\boxed{\text{LL: I've modified the section title in Section 3 to emphasize 2nd-order centered spat. discr. + $\theta$-method}}\\
\boxed{\text{LL: terminology? central differences, centered difference, centered discretizations?}}\\
\boxed{\text{LL: terminology? $L$ spatial discretization matrix, or matrix corresponding to the spat. discr.?}}\\
\boxed{\text{LL: terminology? $M$ full-discretization matrix, or matrix corresponding to the full discr.?}}\\
\boxed{\text{1st page MISSING EMAIL for Yiannis}}
\boxed{\text{Notation to mention: $L$, $M$, $m$}}



Finite difference semi-discretization of \eqref{advection} on a uniform grid
with mesh spacing $\dx$ yields a system of ODEs
\begin{align} \label{semi-discrete}
    u'(t) & = \frac{a}{\dx}Lu(t),
\end{align}
where $u:\mathbb{R}\to\mathbb{R}^m$, $L\in\mathbb{R}^{m\times m}$ is a circulant matrix  \cite[Section 5.16]{matmat}, and $m = 1/\dx$ is the number of grid points.
If one uses an upwind spatial discretization

\boxed{\text{Shall we use a format similar to \eqref{Ldef}}?}
\begin{subequations}
\label{upwind1}
\begin{align} 
...
\section{Algebraic characterization of the entries of the full discretization matrix: $2^\text{nd}$-order centered discretization in space and $\theta$-method in time}\label{section3}
