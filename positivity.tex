...
\begin{align} \label{semi-discrete}
    u'(t) & = \frac{a}{\dx}Lu(t)
\end{align}
where $L$ is a Toeplitz matrix.  It is natural to use an upwdind spatial discretization
\begin{align}
    L_{i,i} & = -1 \ \ \ L_{i,i+1} =  1.
\end{align}
...
of the forward and backward Euler methods, we will study the class of
$\theta$-methods, which include both Euler methods as limiting cases:
\begin{align}
...
\begin{align}
    f_{jk} & = \exp(\imath \xi_k (j-1))/\sqrt{m}  & 1 \le j, k \le m
\end{align}
and $\Lambda$ is the diagonal matrix of eigenvalues, which depends on the
particular finite difference method chosen.  Here $\xi_k$ are evenly
spaced angles such that $\exp(\imath\xi_k)$ are the $m$th roots of unity:
\begin{align}
...
row of $M$:
\begin{align*} 
    M_{11} & = \frac{1}{m} \sum_{\ell=1}^m \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}{1+\theta^2\nu^2\sin^2(\xi_\ell)} \\
...
\begin{align*} 
    M_{11} & = \frac{1}{m} \left(1 + \sum_{\ell=2}^m \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}{1+\theta^2\nu^2\sin^2(\xi_\ell)}\right) \\
...
\end{align*}
We see that $M_{1j}^\infty\ge0$ for all $j>1$, while $M_{11}^\infty\ge0$ if and only if $\theta\ge m/(m-1)$.
