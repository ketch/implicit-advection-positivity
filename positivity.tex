...
\end{split}
\end{align}
where $U:\mathbb{R}\times [0,+\infty)\to\mathbb{R}$ is the unknown function,
$U_0:\mathbb{R}\to\mathbb{R}$ is a given differentiable initial function, and $a>0$ is a constant.
The exact solution of the Cauchy problem \eqref{advection}, given by $U(x,t) =U_0(\{x+a t\})$ (where
$\{\cdot\}$ denotes the fractional part), is positivity preserving; i.e.
...
%\begin{align}
\[
    f_{j,\ell}  \coloneqq \frac{1}{\sqrt{m}}\exp(\imath j \xi_\ell)  \quad\quad (1 \le j, \ell \le m),
\]
%\end{align}
...
%\begin{align}
\[
    \xi_\ell  \coloneqq \frac{2\pi \ell}{m} \quad\quad (1 \le \ell \le m),
\]
...
Thus it is defined completely by the entries of its last row, which
are given by
\begin{align} \label{M-entries}
    M_{m,j} & = \frac{1}{m} \sum_{\ell=1}^m R(\nu\lambda_\ell) \exp(-\imath j \xi_\ell) \quad\quad (1\le j\le m).
\end{align}
...
\begin{split}
	M_{m,j} &= \frac{1}{m} \sum_{\ell=1}^m \frac{1+(1-\theta)\nu \imath\sin(\xi_\ell)}
		{1-\theta\nu \imath\sin(\xi_\ell)}\exp\left(-\imath j \xi_\ell\right) \\
		&= \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)\right)
		\cos(j\xi_\ell) + \nu \sin(\xi_\ell)\sin(j\xi_\ell)}{1+\theta^2\nu^2 \sin^2(\xi_\ell)} \\
		&\quad - \frac{\imath}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)
		\right)\sin(j\xi_\ell) - \nu \sin(\xi_\ell)\cos(j\xi_\ell)}
		{1+\theta^2\nu^2 \sin^2(\xi_\ell)}.
\end{split}
\end{equation}
Note that the angles $\{j\xi_\ell\}_{\ell=1}^m$ are symmetric about the $x$-axis if $m$ is odd, and
also if $m$ is even and $j$ is odd.
If both $m$ and $j$ are even, then the angles are symmetric about the origin.
Therefore, we have that
\begin{align*}
	\sum_{\ell=1}^{m} \sin(j\xi_\ell) = 0 \qquad \text{and} \qquad
		\sum_{\ell=1}^{m} \sin(\xi_\ell)\cos(j\xi_\ell) = 0,
\end{align*}
...
Thus, the imaginary part of \eqref{matrix_entries} vanishes, yielding $M_{m,j}\in\mathbb{R}$ for all $j$,
as expected.
So for $1\le j\le m$ we get
%Since $ M_{m,j}\in\mathbb{R}$, the imaginary part of the above sum vanishes, so for $1\le j\le m$ we also
%have
%\begin{align}\label{firstrow-theta}
\[
  M_{m,j}  = \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)\right)
  \cos(j\xi_\ell) + \nu \sin(\xi_\ell)\sin(j\xi_\ell)}{1+\theta^2\nu^2 \sin^2(\xi_\ell)}.
\]
%\end{align}
...
row of $M$:
\begin{align*}
	M_{m,1} & = \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)\right)
		\cos(\xi_\ell) + \nu \sin^2(\xi_\ell)}{1+\theta^2\nu^2 \sin^2(\xi_\ell)}, \\
	M_{m,m-1} & = \frac{1}{m} \sum_{\ell=1}^m \frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)\right)
		\cos(\xi_\ell) - \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin^2 (\xi_\ell)}, \\
	M_{m,m} & = \frac{1}{m} \sum_{\ell=1}^m \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}
		{1+\theta^2\nu^2\sin^2(\xi_\ell)}.
\end{align*}
...
\begin{align*}
	M_{m,j} = \begin{cases}
				\displaystyle
				\frac{1}{m} \sum_{\ell=1}^{m} \frac{\nu \sin(\xi_\ell)\sin(j\xi_\ell)}
					{1+\theta^2\nu^2 \sin^2 (\xi_\ell)}, &\mbox{if } j \text{ is odd}, \\[20pt]
				\displaystyle
				\frac{1}{m} \sum_{\ell=1}^{m}\frac{\left(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)
					\right)\cos(j\xi_\ell)}{1+\theta^2\nu^2\sin^2(\xi_\ell)}, &\mbox{if } j
					\text{ is even},
			  \end{cases}
\end{align*}
Considering  the above expression for $j = m-1$ we have
\begin{align*}
    M_{m,m-1} & =  \frac{1}{m} \sum_{\ell=1}^{m} \frac{- \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin^2 (\xi_\ell)} < 0.
\end{align*}
%$\bullet$ For \emph{even} $m=2k\ge 4$, consider the above expression for $M_{m,m-1}$. The sum over the first term in the numerator vanishes due to symmetry, and $\sin(\xi_1)=0$, so we have
%\begin{align*}
%    M_{m,m-1} & =  \frac{1}{m} \sum_{\ell=2}^{m} \frac{- \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin^2 (\xi_\ell)} < 0.
%\end{align*}
...
for any $m=2k\ge 4$ we have $M_{m,1}=-M_{m,m-1}$, so that one of
these entries must always be negative.
%\begin{remark}
%By using similar symmetry arguments, one can prove that for any $m=2k\ge 4$ we have
%$M_{m,1}=-M_{m,m-1}$, which also shows that $M\ge 0$ is impossible for $\te\in[0,1]$ and $\nu>0$.
%\end{remark}
...
$2\le\ell\le m$.
 Writing
\begin{align*}
	M_{m,j} &= \frac{1}{m} \left(1 + \sum_{\ell=2}^{m} \frac{(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell))
		\cos(j\xi_\ell) + \nu \sin(\xi_\ell)\sin(j\xi_\ell)}{1+\theta^2\nu^2 \sin^2(\xi_\ell)}
		\right) \quad ( 1 \le j \le m-1), \\
	M_{m,m} &= \frac{1}{m} \left(1 + \sum_{\ell=2}^{m} \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}
		{1+\theta^2\nu^2\sin^2(\xi_\ell)}\right),
\end{align*}
and taking $\nu \to +\infty$ with $m$ and $\te\in(0,1]$ fixed, we find that
\begin{align*} 
    M_{m,j}^\infty &\coloneqq \lim_{\nu \to +\infty} M_{m,j} = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\cos(j\xi_\ell)\right) = \frac{1}{m} \left(1+\frac{1-\theta}{\theta}\right) = \frac{1}{m\theta} \quad\quad ( 1 \le j \le m-1), \\
    M_{m,m}^\infty &\coloneqq \lim_{\nu \to +\infty} M_{m,m} = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\right) =  1-\frac{m-1}{m\theta}.
\end{align*}
We see that
\[M_{m,j}^\infty>0 \text{ for all } 1\le j\le m-1 \text{ and } \te\in(0,1],
\]
 while 
\[M_{m,m}^\infty>0 \quad\Longleftrightarrow\quad \theta>\frac{m-1}{m}.
\]
...
Clearly, we only need to verify the non-negativity of entry $M_{m,m}$ for $\nu>0$ large enough.
In fact, for $\theta=\frac{m-1}{m}$ and for \emph{any} $\nu>0$ we have $M_{m,m}> 0$. To see this, consider a summand with $2\le\ell\le m$ in $M_{m,m}$:
\[
...
\]
and $\varphi(0,\ell)=1$, hence the function $\nu\mapsto M_{m,m}=\frac{1}{m} \left(1 + \sum_{\ell=2}^{m} \varphi(\nu,\ell)\right)$ is positive at $\nu=0$, strictly decreases, and its limit when $\nu\to +\infty$ is $M_{m,m}^\infty=0$, completing the proof of the claim.\\
...
\begin{remark}
It is not restrictive to assume $a>0$ in \eqref{advection}. If we assumed $a<0$ instead, then the results of Theorems \ref{thm1} and \ref{thm2} would remain valid (together with Figures \ref{fig_variousk}--\ref{fig_boundary}, for example), with all the arguments in their proofs being essentially the same.  For example, as we will see in Section \ref{section3}, the non-negativity of (the last row of) matrix $M$ is governed by the elements $M_{m,m}$ and $M_{m,m-1}$ for $a>0$ and $m$ odd---this would change to elements $M_{m,m}$ and $M_{m,1}$ for $a<0$ and $m$ odd.
\end{remark}
...
To illustrate the structure of $M$, we present its last row (as a vector, and with the common denominator of the entries in front of it) for the smallest values of $m$.
\begin{example}\label{example1} 
%For $m=2$, 
%\[\frac{1}{{\theta ^2 \nu ^2}/{4}+1}\left(-\frac{1}{4} \theta ^2 \nu ^2+\frac{\theta  \nu ^2}{4}+1,\theta  \nu -\frac{\nu }{2}\right),\]
For $m=3$ the last row of \eqref{Mdef} is
\[
...
\end{equation}
\begin{remark}
The subscripts of $\cP_{j,k}$ thus refer to the position of the polynomial within the last row of $M$, and the size of $M\in\mathbb{R}^{(2k+1)\times(2k+1)}$, respectively.
\end{remark}
...
\end{equation}
are easily verified. We can now start describing the entries of the last row of $M$.
\begin{remark}
...
\begin{align}
    \lambda_\ell & =
    \begin{cases}
        -\imath \xi_\ell, & 1 \le \ell < m/2, \\
        0, & \ell = m/2 \text{ and } m \text{ is even,} \\
        \imath(2\pi - \xi_\ell), & m/2 < \ell \le m.
    \end{cases}
\end{align}
The entries of the last row of this circulant matrix are again
given by \eqref{M-entries} and take the form
\begin{align}
	M_{m,j}  = \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\lambda_\ell^2\right)
  \cos(j\xi_\ell) + \nu\lambda_\ell\sin(j\xi_\ell)}{1+\theta^2\nu^2\lambda_\ell^2}.
\end{align}
Taking $\theta=1$ (backward Euler)
and recognizing that the imaginary part vanishes, this leads to
\begin{align}
    M_{m,j} & = \frac{1}{m} \sum_{l=1}^{m} \frac{\cos(j\xi_\ell) + \nu \lambda_\ell \sin(j\xi_\ell)}{1+\nu^2 \lambda_{\ell}^2} \\
