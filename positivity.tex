...
Thus, the imaginary part of \eqref{matrix_entries} vanishes and we get $M_{1,j}\in\mathbb{R}$ for all $j$
as expected.
...
\begin{align*}
	M_{1,j} = \begin{cases}
			  ...
			  \end{cases}
\end{align*}
Considering  the above expression for $j = 2$ we have
\begin{align*}
    M_{1,2} & =  \frac{1}{m} \sum_{\ell=2}^{m} \frac{- \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin^2 (\xi_\ell)} < 0.
\end{align*}
...
of $\theta\in[0,1]$ and $\nu>0$.
\begin{remark}
It can be easily seen that for any $m=2k\ge 4$ we have $M_{1,2}=-M_{1,m}$, which also shows that $M\ge 0$
...
\noindent\rule{10cm}{0.4pt}\\
\boxed{\text{SOME INTERPRETATION OF THIS THM IS NEED HERE}}\\
