...
\boxed{\text{TO BE REMARKED HERE OR in the INTRO: although we consider some spatial discretizations that are of higher order,
...
After the spatial discretization matrix $L$ has been chosen, it is coupled with the $\theta$-method as time discretization, and the full-discretization matrix $M$ is obtained (see \eqref{Mdualdef} and \eqref{R}).

\begin{remark} 
In \eqref{Mdualdef}, the matrix $R(\nu L)$ is computationally more difficult to evaluate (due to the presence of the inverse of a non-diagonal matrix), whereas the matrix $R(\nu \Lambda)$ is much easier to evaluate (since one needs to take the inverse of a diagonal matrix only).
\end{remark} 
