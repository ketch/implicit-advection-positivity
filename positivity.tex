...
We have chosen the $\theta$-method (where $\theta\in[0,1]$ is a fixed parameter) as time discretization because it is one of the simplest families of numerical methods, also containing the explicit and implicit Euler schemes.  The explicit and implicit Euler methods are recovered when $\theta=0$ and $\theta=1$, respectively. The $\theta$-method has order 1 for $\theta\ne 1/2$, and has order 2 for $\theta=1/2$.
--------------------------------------------------------------------------------\\

Final sentence: possible future directions: investigate also higher-order time discretizations.
--------------------------------------------------------------------------------\\
...
\boxed{\text{TO BE REMARKED: although we consider some spatial discretizations that are of higher order,
due to the low order of the time discretizations we consider, the total discretization is also of low order.}}

TO BE REMARKED: Here we just make some observations, without proving them.\\


\subsection{Higher-order centered discretizations in space, $\theta$-method in time}

The coefficients of the central differences can be found, e.g., in \cite{bengt}, from which the corresponding circulant matrices $L$ can be constructed.
Here, we consider the first few cases.

The matrix $L$ in \eqref{Ldef} corresponds to a $2^\text{nd}$-order scheme with stencil width 3 (and hence, $m\ge 3$) with entries $(-1/2, 0, 1/2)$ on the central diagonals.

When the stencil width is 5 (implying  $m\ge 5$ and $4^\text{th}$-order), the entries on the central diagonals are $(1/12, -2/3, 0, 2/3, -1/12)$ (with the middle 0s on the main diagonal).

When the stencil width is 7 (implying $m\ge 7$ and $6^\text{th}$-order), the entries on the central diagonals are $(-1/60, 3/20, -3/4, 0, 3/4, -3/20, 1/60)$.

Our computations suggest that the non-negativity properties of the matrix $M$ again depend on the parity of $m$. 

\begin{description}[style=unboxed,leftmargin=0cm]
\item [{Case 1:} $m$ is {even}.] For the $4^\text{th}$-order scheme, we have found that $M\ge 0$ cannot not hold for any $\theta\in[0,1]$ and $\nu>0$ when $m\in\{6,8,10\}$.

For the $6^\text{th}$-order scheme, we checked that $M\ge 0$ does not hold for any $\theta\in[0,1]$ and $\nu>0$ when $m\in\{8,10\}$.

\item [{Case 2:} $m$ is {odd}.] In this case we have found a similar 
\end{description}


\subsection{Fourier spectral collocation in space, $\theta$-method in time}
...
where the matrix $L$ takes the form \boxed{\text{REFERENCE?}}
...
The entries of the first row of this circulant matrix are again given by \eqref{M-entries}, and for the
$\theta$-method they take the form
...
\begin{description}[style=unboxed,leftmargin=0cm]
\item [{Case 1:} $m$ is {even}.]
From \eqref{eq51} we have that \boxed{\text{I DON'T SEE THIS, LL.}}
\begin{align*}
  M_{1,m} = \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\lambda_\ell^2\right)
...
\item [{Case 2:} $m$ is {odd}.]
This time we find a similar behavior observed earlier, see Figures ...
\end{description}
