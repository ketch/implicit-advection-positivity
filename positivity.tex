...
\boxed{\text{Is the spectral collocation method a finite difference semi-discr.? Is it better? Many spatial semi-discr's}}
\begin{align} \label{semi-discrete}
    u'(t) & = \frac{a}{\dx}Lu(t),
\end{align}
where $u:\mathbb{R}\to\mathbb{R}^m$, $L\in\mathbb{R}^{m\times m}$ is a circulant matrix  \cite[Section 5.16]{matmat}, and $\dx = 1/m$ denotes the spatial discretization step size.
...
In Section \ref{otherdisc}:\\
\boxed{\text{Here we just make some observations, without proving them. EARLIER: detailed analysis}}\\
\boxed{\text{Although we consider some spatial discretizations that are of higher order,
