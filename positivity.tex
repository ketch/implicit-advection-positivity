...
U_0(x+at)$, is positivity preserving; i.e.
\begin{align} \label{implies-positivity}
U(x,0) \ge 0 \implies U(x,t) \ge 0.
\end{align}
This property also holds on a finite domain with periodic boundary
conditions, which will be the setting for this work.

Any finite difference semi-discretization of \eqref{advection} yields a system of ODEs
\begin{align} \label{semi-discrete}
    u'(t) & = \frac{a}{\dx}Lu(t)
\end{align}
where $L$ is a Toeplitz matrix.  It is natural to use an upwind spatial discretization
\begin{align}
    L_{i,i} & = -1 \ \ \ L_{i,i+1} =  1.
\end{align}
The exact solution of \eqref{semi-discrete} is also then positivity
preserving, and a corresponding full discretization will be positive
under an appropriate step size restriction
if the forward Euler method or any strong stability preserving
method is used in time.  A more accurate spatial discretization is
obtained with the centered difference
\begin{align} \label{centered-difference}
    L_{i,i-1} & = -1/2 \ \ \ L_{i,i+1} =  1/2.
\end{align}
However, this semi-discretization is not positivity preserving (proof?).
This implies that any consistent full discretization based on \eqref{centered-difference}
must fail to preserve positivity under sufficiently small step sizes.
...
On the other hand, using \eqref{centered-difference} with backward
Euler time integration, one generally observes positivity preservation
under large step sizes.  To investigate the differences between the behavior
of the forward and backward Euler methods, we will study the one-parameter family of
$\theta$-methods for $0\le\te\le1$, which include both Euler methods as limiting cases:
\begin{align}
    u^{n+1} = u^n + \frac{a\dt}{\dx}((1-\theta)Lu^n + \theta Lu^{n+1}).
\end{align}
Recall that $\te=0$ and $\te=1$ correspond to the forward (or explicit) and backward (or implicit) Euler methods, respectively, and $\te=1/2$ yields the only second-order method in the $\te$-family. 
...
in space leads to \eqref{semi-discrete}, where $L$ is a circulant matrix
...
\begin{align}
    f_{j,\ell} & = \exp(\imath \xi_\ell (j-1))/\sqrt{m}  & 1 \le j, \ell \le m
\end{align}
...
Products and inverses of circulant matrices are also circulant, so
...
\begin{align} \label{M-entries}
    M_{1,j} & = \frac{1}{m} \sum_{\ell=1}^m R(\nu\lambda_\ell) \exp(-i(j-1)\xi_\ell).
\end{align}

\subsection{Centered in space, $\theta$ method in time}
...
In what follows we assume $m\ge 3$. For the $\theta$-method we have
\begin{align}
...
\begin{theorem}
Consider the advection equation with periodic boundary conditions discretized using centered
...
Here we assume that $a>0$ is a positive constant.\\

Relation to SSP analysis?\\
