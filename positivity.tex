...
When this matrix $L$ is coupled with the $\theta$-method as time discretization, the full discretization matrix $M$ is obtained.  It is well known and easily shown that, as with the discretizations considered above,
...
which is negative for $\nu>0$ sufficiently \emph{small} and $\theta\in(0,1]$ arbitrary, since $\lambda_1=\lambda_{m/2+1}=0$, and $\lambda_\ell \sin(\xi_\ell)>0$ for the remaining indices $\ell$. 
 ...
 Meanwhile, for $\nu \to \infty$, since
$$
(1+\theta^2 \nu^2 \lambda_\ell^2)^{-1} = (\theta^2 \nu^2 \lambda_\ell^2)^{-1}(1+{\mathcal O}(\nu^{-2})),
$$
we have
\begin{align*}
    M_{1,m} & = \frac{1}{m}(\cos(\xi_1)+\cos(\xi_{m/2+1})) + \frac{1+{\mathcal O}(\nu^{-2})}{m} \sum_{\ell=2, \ell\ne m/2+1}^m \left(\frac{-1}{\theta}\cos(\xi_\ell) - \frac{\sin(\xi_\ell)}{\nu\theta^2\lambda_\ell}\right) \\
              & =  -\frac{1}{m} \sum_{\ell=1}^m  \frac{\sin(\xi_\ell)}{\nu\theta^2\lambda_\ell}+ {\mathcal O}(\nu^{-3}),
\end{align*}
which is negative again for large enough $\nu$ and $\theta\in(0,1]$ arbitrary. \\ 
------------------------------------------------\\
...
\]
Let us fix $\theta\in(0,1]$ arbitrary and consider this last expression. Then it is clear that (i) the first sum in $[\ldots]$ is 0; (ii) ${\sin(\xi_\ell)}/{\lambda_\ell}>0$ for each $\ell$ appearing in the second sum; and (iii) the expression in $\{\ldots\}$  for each $\ell$ can be made arbitrarily small in modulus by choosing $\nu>0$ suitably large. This means that for any $\theta\in(0,1]$, $M_{1,m}<0$ for \emph{large} enough $\nu>0$. \\
------------------------------------------------
