...
\end{align*}
Consider the expression for $M_{1,m}$.
...
Let us now consider the case of odd $m$.  Writing
\begin{align*} 
    M_{1,1} & = \frac{1}{m} \left(1 + \sum_{\ell=2}^m \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}{1+\theta^2\nu^2\sin^2(\xi_\ell)}\right) \\
    M_{1,j} & = \frac{1}{m} \left(1 + \sum_{\ell=2}^m \frac{(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell))\cos((j-1)\xi_\ell)- \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin((j-1)\xi_\ell)\sin(\xi_\ell)}\right) & j>1,
\end{align*}
and taking $\nu \to \infty$ with $m$ fixed, we find that
\begin{align*} 
    M_{1,1}^\infty := \lim_{\nu \to \infty} M_{11} & = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\right) =  \frac{1}{m} \left(1 - (m-1) \frac{1-\theta}{\theta}\right)\\
    M_{1,j}^\infty := \lim_{\nu \to \infty} M_{1,m} & = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\cos((j-1)\xi_\ell)\right) = \frac{1}{m} \left(1+\frac{1-\theta}{\theta}\right) = \frac{1}{m\theta}.
\end{align*}
We see that $M_{1,j}^\infty\ge0$ for all $j>1$, while $M_{1,1}^\infty\ge0$ if and only if $\theta\ge m/(m-1)$.
...
(ii) if $m$ is odd and $\theta\ge m/(m-1)$, then for large enough $\dt$ all entries of $M$ are nonnegative.
\end{theorem}
...
\section{An algebraic characterization of the matrix entries}
...
\begin{center} Find (necessary or sufficient) conditions for a set $\cS:=\{\sigma_1,\ldots,\sigma_m\}\subset\mathbb{C}$ to be the spectrum of some non-negative $m \times m$ matrix.
\end{center}
It is known \cite[Chapter 4]{nonnegmatr} that if $\cS$ is the spectrum of 
...
for $m=5$ and $\te=1$, for example, \eqref{sigmampq} with  $p\in\{1,\ldots,9\}$, $q\in\{2,3\}$ yields the lower bounds
...
\end{array}
\]
The true lower bound, according to Theorem \ref{thm2} with $k=2$ and $\te=1$, is
\[
\nu\ge \nu_R(2,1)\approx 4.4111.
\]
\begin{remark}
In the actual computations of the table values, we made use of the equality 
\[
...
\]
The resulting high-degree inequalities in $\nu$ have then been solved (for $p=9$, $q=3$, for example, the rational functions in $\nu$ on both sides have numerator and denominator degrees 108).
\end{remark}
\begin{remark}
...
It is possible to sharpen the lower bounds \eqref{nustartpq} by making use of some more specific results. For simplicity, we still consider the case $m=5$, $\te=1$. The inequality \eqref{sigmampq} holds for any non-negative matrix. However, we know in addition that the matrix $M(5,1,\nu)$ is circulant. This leads us to the realm of 
\emph{structured non-negative inverse eigenvalue problems}. The spectra of non-negative circulant matrices 
have been characterized in \cite[Theorem 10]{rojosoto}. 
