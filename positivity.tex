...
To return to the original variables $(\te,\nu)$ from the variable $y$, by  \eqref{fromytonu} we define
...
\end{cases}\]
The value $+\infty$ here is introduced for convenience so as to make our later descriptions shorter.
A reformulation of Corollary \ref{cor3} is given below.
\begin{corollary}
...
\end{corollary}
\begin{proof}
Then for fixed $k$ and $\te$, $\Por\ge 0$ is equivalent to $\nu\ge\nur$, since the map $(0,1)\ni y\mapsto\frac{2y}{1-y^2}\in(0,+\infty)$ in \eqref{fromytonu} is a strictly increasing bijection.
\end{proof}
\begin{remark}
From \eqref{yrasympt} we get that
\[
