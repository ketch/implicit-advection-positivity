...
Finite difference semi-discretization of \eqref{advection} on a uniform grid $\{\dx,2\dx,\ldots, m\dx\}\subset [0,1]$ 
with mesh spacing $\dx>0$ and $m\dx=1$ yields a system of ODEs
\begin{align} \label{semi-discrete}
    u'(t) & = \frac{a}{\dx}Lu(t),
\end{align}
where $u:\mathbb{R}\to\mathbb{R}^m$, $L\in\mathbb{R}^{m\times m}$ is a circulant matrix  \cite[Section 5.16]{matmat}, and $m \in\mathbb{N}^+$ is the number of grid points (due to the periodic boundary condition, the grid points $0$ and $1$ are identified).
...
The upwind discretization \eqref{upwind1} is only of order one.
...
such as the centered difference method \eqref{centered-difference} and the
...
In the later sections of the paper, we combine the $\theta$-method in time
with high-order methods in space, as a next step in this direction.
...
In Section \ref{sectiondiscFourier} we study positivity of the $\theta$-method in
time combined with second-order centered differences in space, using discrete Fourier
analysis.  In Section \ref{section3}, we study this discretization in more detail. 
We derive an explicit formula for the matrix entries in Section \ref{explsect}
and use that formula to provide precise results on its positivity in Section \ref{nonnegsect}.
...
The symbol $M\ge 0$ means that  $M_{i,j}\ge 0$ for every entry $1\le i, j\le m$ of the matrix $M\in\mathbb{R}^{m\times m}$. The positive integer $m$ is the number of spatial grid points within the interval $[0,1]$, and the matrices $L$ and $M$ are the matrices corresponding to the spatial and the full discretizations, respectively. 
Finally, as is customary in the context of space-time discretizations of partial differential equations, superscripts of $u$, e.g.~in \eqref{firsttheta}, are not exponents but denote time discretization steps.  

\boxed{\text{ABSTRACT}}\\

\boxed{\text{periodic boundary condition VS periodic boundary conditions?}}\\

\boxed{\text{This was put after Theorem 1, delete?}}
...
\begin{theorem}[About the full discretiation matrix corresponding to $2^{\text{nd}}$-order centered discretization in space and $\theta$-method in time]\label{thm2}
\begin{itemize}\ 
...
The coefficients of the centred differences can be found, e.g., in \cite{bengt}, from which the corresponding circulant matrices $L$ describing the spatial discretization can be constructed.
...
\end{itemize}
We can obtain an eigendecomposition \eqref{eigen} with eigenvectros given by \eqref{eigenvectrors} and
eigenvalues
\begin{align*}
...
For instance, the vector $C$ is equal to $(1/2)$, $(2/3,-1/12)$, and $(3/4,-3/20,1/60)$ for stencil width
...
The entries of the first row of the circulant matrix $M$ are again given by \eqref{M-entries}, and now %for the $\theta$-method 
they take the form
...
\begin{description}[style=unboxed,leftmargin=0cm]
\item [{Case 1:} $m$ is {even}.]
From \eqref{eq51} we have (using \eqref{angle-identities}) that
\begin{align*}
  M_{1,m} = \frac{1}{m} \sum_{\ell=1}^{m} \frac{\left(1-\theta(1-\theta)\nu^2\lambda_\ell^2\right)
...
and dividing by $\nu$, we can write the numerator as a univariate polynomial in
$p_m(y)$.  For example,
...
In this work we have studied the positivity of certain fundamental
discretizations of the advection equation.  We have employed a direct
algebraic approach, first based on discrete Fourier analysis and then
on a polynomial representation.  We emphasize that our
goal is not to directly provide efficient discretizations but rather
...
For all discretizations studied, we have found similar behavior.
If the number of grid points $m$ is even, no method is positivity
...
of the CFL number $\nu$, larger values of $\theta$, and smaller
values of $m$.  These tendencies, and more specific results, are described
...
Our detailed analysis in Sections \ref{sectiondiscFourier}-\ref{section3}
was devoted to the $\theta$-method in time and second-order centered
differences in space.  The characterization of the full discretization
matrix for this case, along with the related trigonometric identities
...
discretizations of other linear PDEs could be analyzed with the same techniques.
Further areas for extension might incorporate other boundary conditions or
