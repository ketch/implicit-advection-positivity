...
\begin{description}[style=unboxed,leftmargin=0cm]
\item [{Case 1:} $m$ is {even}.] For the $4^\text{th}$-order scheme, we have found (by using symbolic calculations) that $M\ge 0$ cannot not hold for any $\theta\in[0,1]$ and $\nu>0$ when $m\in\{6,8,10\}$.

For the $6^\text{th}$-order scheme, we checked (again symbolically) that $M\ge 0$ does not hold for any $\theta\in[0,1]$ and $\nu>0$ when $m\in\{8,10\}$.
...
\item [{Case 2:} $m$ is {odd}.]
This time we find a similar behavior observed earlier, see Figure ...
\end{description}
