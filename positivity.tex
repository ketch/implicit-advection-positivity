...
\end{align}
\end{subequations}
where $U:\mathbb{R}\times [0,+\infty)\to\mathbb{R}$ is the unknown function, $U_0:\mathbb{R}\to\mathbb{R}$ is a given differentiable initial function, and $a>0$ is a constant. \boxed{\text{LL WHAT IF } a<0?}
...
condition, which will be the setting for this work. \boxed{\text{WE NEED A SENTENCE about the significance of positivity preservation}}
...
\end{align}
\boxed{\text{WHAT WHEN i=m?}} then the exact solution of \eqref{semi-discrete} is also positivity
...
\end{align}
\boxed{\text{WHAT WHEN i=m?}} However, this semi-discretization is not positivity preserving, because the matrix $L$ has at least one negative off-diagonal entry \cite[Chapter I, Theorem 7.2]{hundsdorferverwer}.
...
The results obtained in this section are independent of the question of non-negativity, and may be applicable in other contexts as well.  \boxed{\text{LL POINT OUT: Corollary 1, asymptotic bounds}}
...
\boxed{\text{WE NEED TO SPECIFY EXACTLY IN WHICH WAY WE SUBDIVIDE $[0,1]$ BY $m$ DISCRETIZATION POINTS.}}\\
...
\begin{align}
    f_{j,\ell} & := \frac{1}{\sqrt{m}}\exp(\imath  (j-1) \xi_\ell)  & 1 \le j, \ell \le m,
\end{align}
...
\begin{align}
    \xi_\ell & := \frac{2\pi(\ell-1)}{m} & 1 \le \ell \le m.
\end{align}
...
Consider the case of a 3-point centered difference \boxed{\text{SCHEME/APPROXIMATION?}} in space:
\begin{align}
...
\right).
\end{equation}
\boxed{\text{I FEEL THIS PARAGRAPH HAS BECOME A LITTLE REDUNDANT?}} The exact solution of the resulting semi-discrete
...
In what follows we assume $3\le m\in\nplus$. The stability function $R$ for the $\theta$-method is \cite[Chapter IV.3]{hairerwanner}
\begin{align}\label{R}
...
$\bullet$ For \emph{even} $m=2k\ge 4$, consider the expression for $M_{1,m}$. The sum over the first term in the numerator vanishes due to symmetry and we have
\begin{align*}
...
M_{1,2}=-M_{1,2k},
\]
from which one can also easily deduce that $M\ge 0$ is impossible for $\te\in[0,1]$ and $\nu>0$.
\end{remark}

$\bullet$ Let us now consider the case of \emph{odd} $m=2k+1\ge 3$.  Writing
\begin{align*} 
    M_{1,1} & = \frac{1}{2k+1} \left(1 + \sum_{\ell=2}^{2k+1} \frac{1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell)}{1+\theta^2\nu^2\sin^2(\xi_\ell)}\right) \\
    M_{1,j} & = \frac{1}{2k+1} \left(1 + \sum_{\ell=2}^{2k+1} \frac{(1-\theta(1-\theta)\nu^2\sin^2(\xi_\ell))\cos((j-1)\xi_\ell)- \nu \sin^2 (\xi_\ell)}{1+\theta^2\nu^2 \sin((j-1)\xi_\ell)\sin(\xi_\ell)}\right), & j\ge 2,
\end{align*}
and taking $\nu \to +\infty$ with $m=2k+1$ and $\te\in(0,1]$ fixed, we find that
\begin{align*} 
    M_{1,1}^\infty := \lim_{\nu \to +\infty} M_{1,1} & = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\right) =  \frac{1}{m} \left(1 - (m-1) \frac{1-\theta}{\theta}\right)\\
    M_{1,j}^\infty := \lim_{\nu \to +\infty} M_{1,j} & = \frac{1}{m} \left(1 - \sum_{\ell=2}^m \frac{1-\theta}{\theta}\cos((j-1)\xi_\ell)\right) = \frac{1}{m} \left(1+\frac{1-\theta}{\theta}\right) = \frac{1}{m\theta},& j\ge 2.
\end{align*}
We see that $M_{1,j}^\infty\ge0$ for all $2\le j\le m$, while $M_{1,1}^\infty\ge0$ if and only if $\theta\ge\frac{m-1}{m}$.
...
(ii) if $m$ is odd and $\theta\in\left[\frac{m-1}{m},1\right]$, then for large enough $\dt$ all entries of $M$ are non-negative.
\end{theorem}


\boxed{\text{MENTION THE INTERPRETATION OF THM 1:}}\\
\boxed{\text{large enough $\dt$ if $\dx$ is fixed, or small enough $\dx$ if $\dt$ is chosen before carrying out spatial discretization}}\\


\boxed{\text{MOREOVER, MAYBE WE SHOULD EMPHASIZE HERE AGAIN THE FOLLOWING}}\\
\[
\text{positivity preservation of the fully discrete numerical solution}\quad \Longleftrightarrow \quad M\ge 0.
\]

A refinement of Theorem \ref{thm1} will be given at the end of Section \ref{section3}, see Theorem \ref{thm2}.
...
It is trivial that for $\te=0$ we have $M(m,0,\nu)=I+\nu L$, hence $M\ge 0$ cannot hold for any $\nu>0$. The case $m=2k$ has been discussed in Section \ref{sectiondiscFourier}. Thus, throughout the rest of this section, we can assume that 
...
\boxed{\text{MAYBE dknotes.pdf Section 5?}}


Below we point out a related approach to studying the non-negativity of the family of matrices $M$. Due to the fact that the eigenvalues of $M$ in \eqref{Mdualdef} are known, we can use some conditions developed in the context of \emph{non-negative inverse eigenvalue problems}, where the main question is the following. 
...
\end{array}
\]
The necessary condition \eqref{sigmampq}, valid for \emph{any} non-negative matrix, already 
...
It is possible to sharpen the lower bounds in \eqref{nustartpq} by making use of some more specific results. For simplicity, we still consider the case $m=5$, $\te=1$. The inequality \eqref{sigmampq} holds for \emph{any} non-negative matrix. However, we know in addition that the matrix $M(5,1,\nu)$ is \emph{circulant}. This leads us to the realm of 
\emph{structured non-negative inverse eigenvalue problems}. The spectra of, for example, non-negative circulant matrices have been characterized in \cite[Theorem 10]{rojosoto}. For simplicity, we do not present that theorem here,  only give some illustrations. 

\boxed{\text{LL COMPLETE}}
